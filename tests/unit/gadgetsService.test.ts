...
describe('GadgetsService', () => {
    ...
    describe('getAllGadgets', () => {
        ...
        it('should throw an error if there is an issue retrieving gadgets', async () => {
            gadgetRepository.findAll.mockRejectedValue(
                new Error('Database error')
            )

            await expect(gadgetsService.getAllGadgets()).rejects.toThrow(
                'Database error'
            )
            expect(gadgetRepository.findAll).toHaveBeenCalledTimes(1)
        })
    ...
    describe('createGadget', () => {
        it('should create and return a new gadget', async () => {
            const dto: CreateGadgetDto = {
                ...
                brand: 'Brand C',
                releaseDate: new Date().toISOString(),
                description: 'A new gadget description',
            }
            const createdGadget: Gadget = {
                id: '3',
                name: dto.name,
                brand: dto.brand,
            ...
            expect(gadgetRepository.create).toHaveBeenCalledWith(
        ...
        it('should throw an error if name is missing', async () => {
            ...
            await expect(gadgetsService.createGadget(dto)).rejects.toThrow(
                'Name is required'
            )
        })
        ...
        it('should throw an error if gadget already exists', async () => {
            const dto: CreateGadgetDto = {
                ...
                brand: 'Brand C',
                releaseDate: new Date().toISOString(),
                description: 'An existing gadget description',
            }
            gadgetRepository.create.mockRejectedValue(
                new Error('Gadget already exists')
            )
            await expect(gadgetsService.createGadget(dto)).rejects.toThrow(
